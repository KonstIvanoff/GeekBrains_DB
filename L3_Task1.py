...
def parse_salary(sal):
        ...
        # print(sal, mystr, res)
        if len(res) == 2:
                c1, c2 = float(res[0]), float(res[1])
        elif len(res) == 1:
                try:
                        i = mystr.index('от')
                        c1, c2 = float(res[0]), None
                except ValueError:
                        c1, c2 = float(res[0]), float(res[0])
...
hh = hh_parsing('T-SQL','hhcc.json')
hh.parse()

# sj = superjob_parsing('T-SQL','sjcc.json')
# sj.parse()
